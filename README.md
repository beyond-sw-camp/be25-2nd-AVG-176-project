...
| 이진 |  도메인 지식과 실제 업무 흐름에 대한 이해가 충분하지 않은 상태에서 기획에 의존해 개발을 진행하다 보니 구현 과정에서 요구사항 명세서와 화면 설계서가 실제 처리 프로세스 사이에 차이가 있다는 것을 기능을 구현한 뒤에 알게 되었습니다. 또한 정책, 가공, 등록 기능뿐만 아니라 주문, 결제, 배송기능까지 이어지는 서비스 흐름에서 다른 팀원들의 기능과 어떻게 연결되는지도 함께 이해해야 한다는 것을 깨달았습니다. 이번 경험을 통해 기능을 구현할 떄에 담당 파트만 개발하는 것이 아니라 서비스 전체의 흐름과 연계성을 함께 고려해야 하며 애매한 부분은 임의로 처리하기보다 기획자와 팀원들과 충분히 소통하고 사전에 검토하는 과정이 매우 중요하다는 점을 배웠습니다. 또한 MSA 방식을 처음 시도 해보면서 단일 기능 구현뿐만 아니라 서비스 간 역할 분리와 서버 구조를 이해해볼 수 있었습니다.|
| 이용호 | 이번 프로젝트에서 저는 MSA 방식으로 서버를 분리해 구축하는 과정에 참여했고, 주문과 배송 기능 구현을 맡았습니다. 하나의 서버에서 모든 기능을 처리하는 방식이 아니라 서비스별 역할을 나누어 개발하다 보니, 기능 구현뿐 아니라 서비스 간 연결과 구조를 함께 고민해야 했습니다. 주문 기능에서는 주문 생성과 조회, 상태 관리 흐름을 구현했고, 배송 기능에서는 배송 생성, 주문별 배송 조회, 배송 상태 변경 API를 구현했습니다. 특히 주문 이후 결제와 배송으로 이어지는 흐름을 연결하면서, 각 기능이 독립적이면서도 유기적으로 동작해야 한다는 점을 배웠습니다. 또한 이번 프로젝트에서 백엔드와 프론트엔드를 실제로 연동해보는 경험도 처음 해봤습니다. API를 만드는 것에서 끝나는 것이 아니라, 프론트에서 어떤 형태의 데이터를 필요로 하는지 맞춰 주고 응답 구조와 요청 방식까지 함께 고려해야 한다는 점을 알 수 있었습니다. 이 과정을 통해 백엔드 개발은 서버 로직만 잘 만드는 것이 아니라, 프론트와 원활하게 연결될 수 있도록 설계하는 것도 중요하다는 것을 배웠습니다. 브랜치에 프로젝트를 추가하는 과정에서 협업 환경에 맞는 브랜치 관리와 프로젝트 구조 정리의 중요성도 체감했습니다. 이번 프로젝트를 통해 단순히 API를 만드는 것을 넘어, MSA 환경에서 서비스의 역할과 연결 구조를 고려하며 개발하는 경험을 할 수 있었습니다. |
| 현재진 | 이번 프로젝트에서 저는 팀원들과 함께 MSA 방식으로 서버를 분리해 구축하는 과정에 참여했고, 주문 기능 일부와 결제 기능 구현을 맡았습니다. 서비스가 분리된 구조에서 개발을 진행하다 보니, 단순히 기능 하나를 구현하는 것을 넘어 다른 도메인과 어떻게 연결되는지까지 함께 고려해야 했습니다. 주문 기능 일부를 구현하면서는 전체 주문 흐름 안에서 제가 맡은 기능이 어떤 역할을 하는지 이해하는 것이 중요했습니다. 특히 결제 기능은 주문과 직접 연결되는 핵심 기능이었기 때문에, 결제 요청 처리와 상태 관리, 카드 정보와의 연계를 신중하게 다뤄야 했습니다. 이 과정을 통해 하나의 기능이 독립적으로 동작하는 것만큼, 전체 서비스 흐름 안에서 자연스럽게 이어지는 것이 중요하다는 점을 배웠습니다. 구현 과정에서는 데이터 무결성과 예외 처리의 중요성도 크게 느꼈습니다. 특히 결제와 카드 정보가 연결되어 있는 구조에서는 작은 로직 하나가 다른 기능에 영향을 줄 수 있기 때문에, 기능 구현뿐 아니라 안정성까지 함께 고려해야 했습니다. 이번 프로젝트를 통해 MSA 환경에서 협업하며 기능을 구현하는 경험을 할 수 있었고, 주문과 결제 기능이 실제 서비스 흐름 안에서 어떻게 연결되는지 이해할 수 있었습니다. 단순히 코드를 작성하는 것을 넘어, 다른 팀원들과 역할을 나누고 함께 하나의 서비스를 완성해가는 과정 자체가 의미 있는 경험이었다고 생각합니다. |
